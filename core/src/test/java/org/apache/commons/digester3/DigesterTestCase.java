...
/**
 * <p>
 * Test Case for the Digester class. These tests exercise the individual methods
 * of a Digester, but do not attempt to process complete documents.
 * </p>
 *
...
public class DigesterTestCase {
	 ...
	 * versions of the DTDs that we know about. There <strong>MUST</strong> be an
	 * even number of Strings in this array.
	 */
	...
	/**
	 * Test <code>null</code> parsing. (should lead to
	...
	public void testRulesBase() {
		...
		// Test exact match
		assertEquals("Exact match takes precedence 1", 1,
				digester.getRules().match(null, "a/b/c/d", null, null).size());
	...
	public void testOnceAndOnceOnly() throws Exception {
		...
		final Digester digester = spy(Digester.class);

		final String xml = "<?xml version='1.0'?><document/>";
		digester.parse(new StringReader(xml));

		verify(digester, times(1)).initialize();
	}

	@Test
	public void testBasicSubstitution() throws Exception {
		final Rule tsr = spy(Rule.class);
		String[] tsrBody = new String[1];
		Attributes[] tsrAttributes = new Attributes[1];
		doAnswer((stubInvo) -> {
			String text = stubInvo.getArgument(2);
			tsrBody[0] = text;
			return null;
		}).when(tsr).body(any(), any(), any());
		doAnswer((stubInvo) -> {
			Attributes attributes = stubInvo.getArgument(2);
			tsrAttributes[0] = new AttributesImpl(attributes);
			return null;
		}).when(tsr).begin(any(), any(), any());
		final Digester digester = new Digester();
		digester.addRule("alpha/beta", tsr);

		// it's not easy to transform dirty harry into the mighty circus - but let's
		// give it a try
		final String xml = "<?xml version='1.0'?><alpha><beta forname='Dirty' surname='Harry'>Do you feel luck punk?</beta></alpha>";
		InputSource in = new InputSource(new StringReader(xml));

		digester.parse(in);

		assertEquals("Unsubstituted body text", "Do you feel luck punk?", tsrBody[0]);
		assertEquals("Unsubstituted number of attributes", 2, tsrAttributes[0].getLength());
		assertEquals("Unsubstituted forname attribute value", "Dirty", tsrAttributes[0].getValue("forname"));
		assertEquals("Unsubstituted surname attribute value", "Harry", tsrAttributes[0].getValue("surname"));
		...
		assertEquals("Substituted body text", "And now for something completely different...", tsrBody[0]);
		assertEquals("Substituted number of attributes", 1, tsrAttributes[0].getLength());
		assertEquals("Substituted python attribute value", "Cleese", tsrAttributes[0].getValue("", "python"));
	}
	...
	public void testStackAction() {
		final StackAction action = mock(StackAction.class);
		ArrayList<String> actionEvents = new ArrayList<String>();
		when(action.onPop(any(Digester.class), any(String.class), any(Object.class))).thenAnswer((stubInvo) -> {
			String stackName = stubInvo.getArgument(1);
			Object o = stubInvo.getArgument(2);
			final String msg = "pop:" + stackName + ":" + o.toString();
			actionEvents.add(msg);
		...
		when(action.onPush(any(Digester.class), any(String.class), any(Object.class))).thenAnswer((stubInvo) -> {
			String stackName = stubInvo.getArgument(1);
			Object o = stubInvo.getArgument(2);
		...
		assertEquals(0, actionEvents.size());
		d.push(obj1);
		d.push(obj2);
		d.push(obj3);
		d.push(obj4);
		...
		assertEquals(12, actionEvents.size());
		assertEquals("push:null:obj1", actionEvents.get(0));
		assertEquals("push:null:obj2", actionEvents.get(1));
		assertEquals("push:null:replpop.obj3", actionEvents.get(2));
		assertEquals("push:null:replpush.obj4", actionEvents.get(3));
		assertEquals("pop:null:replpush.obj4", actionEvents.get(4));
		assertEquals("pop:null:replpop.obj3", actionEvents.get(5));
		assertEquals("pop:null:obj2", actionEvents.get(6));
		assertEquals("pop:null:obj1", actionEvents.get(7));

		assertEquals("push:stack1:obj8", actionEvents.get(8));
		assertEquals("push:stack1:obj9", actionEvents.get(9));
		assertEquals("pop:stack1:obj9", actionEvents.get(10));
		assertEquals("pop:stack1:obj8", actionEvents.get(11));
	}
}
